...
function iziAlert(status, title, position = "topRight", time = 2000) {
  const options = { position, title, time, displayMode: "replace" };
  const alerts = {
...
var botones = [
  {
    cabecera: "Editar",
    className: "fa fa-pencil-alt btn btn btn-sm btn-primary",
    id: "Editar",
  },
  {
    cabecera: "Anular",
    className: "fa fa-minus-circle btn btn-sm btn-danger",
    id: "Eliminar",
  },
];
...
function GrillaScroll(
  ...
  function crearTabla() {
    ...
    var contenido = `
    <div class='table-container card-body' style='height:${altoGrilla}em;'>
      <div>
    ...
    contenido += `
      <table class='table text-white table-scroll' id='tbl${divGrilla}'>
        <thead>
          <tr class='FilaHead'>
    `;
...
function Tree2(div, listaCabecera, listaDetalle) {
  ...
  for (var j = 0; j < nCaberaCampos; j++) {
    ...
    if (j == 0) {
      html += "<div>";
      html +=
        "<span id='spnCab' class='CabExpandir NoImprimir' class='Centrado'>+</span>";
      html += "<span>";
